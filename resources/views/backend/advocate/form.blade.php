            ...
            <div class="widget-content widget-content-area simple-tab">
                ...
                <div class="tab-content" id="simpletabContent">
                    <div class="tab-pane fade show active" id="general-info" role="tabpanel"
                        ...
                        <x-form class="row" action="admin.advocate" data="{{ $advocate->id ?? '' }}">
                            <x-input.text class="col-md-4" label="First Name" name="first_name"
                                value="{{ $advocate->first_name ?? '' }}" id="first_name" placeholder="Enter first name" />

                            <x-input.text class="col-md-4" label="Last Name" name="last_name"
                                value="{{ $advocate->last_name ?? '' }}" id="last_name" placeholder="Enter last name" />

                            <x-input.tel class="col-md-4" label="Mobile No" name="mobile"
                                value="{{ $advocate->mobile ?? '' }}" id="mobile" placeholder="Enter mobile no" />

                            <x-input.email class="col-md-4" label="Email" name="email" value="{{ $advocate->email ?? '' }}"
                                id="email" placeholder="Enter email" />

                            <x-input.date class="col-md-4" label="Birth of Date" name="birthday"
                                value="{{ $advocate->birthday ?? '' }}" id="birthday" placeholder="Enter birth of date" />

                            <x-input.select class="col-md-2" label="Blood Group" name="blood_group" value=""
                                id="blood_group" placeholder="Select Blood Group">
                                <option value="A+" {{ __select('blood_group', 'A+', $advocate->blood_group ?? '') }}> A+
                                </option>
                                <option value="A-" {{ __select('blood_group', 'A-', $advocate->blood_group ?? '') }}>A-
                                </option>
                                <option value="B+" {{ __select('blood_group', 'B+', $advocate->blood_group ?? '') }}>B+
                                </option>
                                <option value="B-" {{ __select('blood_group', 'B-', $advocate->blood_group ?? '') }}>B-
                                </option>
                                <option value="AB+" {{ __select('blood_group', 'AB+', $advocate->blood_group ?? '') }}>AB+
                                </option>
                                <option value="AB-" {{ __select('blood_group', 'AB-', $advocate->blood_group ?? '') }}>AB-
                                </option>
                                <option value="O+" {{ __select('blood_group', 'O+', $advocate->blood_group ?? '') }}>O+
                                </option>
                                <option value="O-" {{ __select('blood_group', 'O-', $advocate->blood_group ?? '') }}>O-
                                </option>
                            </x-input.select>
                            ...
                            <x-input.select class="col-md-2" label="Religion" name="religion" value=""
                                ...
                                <option value="Islam" {{ __select('religion', 'Islam', $advocate->religion ?? '') }}>Islam
                                </option>
                                <option value="Christianity"
                                    {{ __select('religion', 'Christianity', $advocate->religion ?? '') }}>
                                    Christianity</option>
                                <option value="Hinduism" {{ __select('religion', 'Hinduism', $advocate->religion ?? '') }}>
                                    Hinduism</option>
                                <option value="Buddhism" {{ __select('religion', 'Buddhism', $advocate->religion ?? '') }}>
                                    Buddhism</option>
                                <option value="Judaism" {{ __select('religion', 'Judaism', $advocate->religion ?? '') }}>
                                    Judaism</option>
                                <option value="Sikhism" {{ __select('religion', 'Sikhism', $advocate->religion ?? '') }}>
                                    Sikhism</option>
                                <option value="Other" {{ __select('religion', 'Other', $advocate->religion ?? '') }}>Other
                                </option>
                            </x-input.select>

                            <x-input.select class="col-md-4" label="Gender" name="gender" value=""
                                id="gender" placeholder="Select Gender">
                                <option value="Male" {{ __select('gender', 'Male', $advocate->gender ?? '') }}>Male
                                </option>
                                <option value="Female" {{ __select('gender', 'Female', $advocate->gender ?? '') }}>Female
                                </option>
                                <option value="Others" {{ __select('gender', 'Others', $advocate->gender ?? '') }}>Others
                                </option>
                            </x-input.select>

                            <x-input.select class="col-md-4" label="Nationality" name="nationality" value=""
                                id="nationality" placeholder="Select Nationality">
                                <option value="0" {{ __select('nationality', 0, $advocate->nationality ?? '') }}>
                                    Bangladeshi</option>
                                <option value="1" {{ __select('nationality', 1, $advocate->nationality ?? '') }}>Others
                                </option>
                            </x-input.select>
                            ...
                            <x-input.select class="col-md-4" label="Mother Tongue" name="mother_tongue" value=""
                                ...
                                <option value="Bangla"
                                    {{ __select('mother_tongue', 'Bangla', $advocate->mother_tongue ?? '') }}>Bangla</option>
                                <option
                                    value="English"{{ __select('mother_tongue', 'English', $advocate->mother_tongue ?? '') }}>
                                    English</option>
                                <option
                                    value="Others"{{ __select('mother_tongue', 'Others', $advocate->mother_tongue ?? '') }}>
                                    Others</option>
                            </x-input.select>

                            <div class="col-md-12">
                                <button type="submit"
                                    class="float-right btn btn-info">{{ $advocate->id ?? '' ? 'Update' : 'Create' }}</button>
                            </div>
                        </x-form>
                    </div>
                    ...
                    <div class="tab-pane fade" id="educational-info" role="tabpanel"
                        ...
                        @isset($advocate)
                            @foreach ($advocate->education as $edu)
                                <x-form class="row" action="admin.education-info" data="{{ $edu->id ?? '' }}">

                                    <input type="hidden" name="user_id" value="{{ $advocate->id ?? '' }}">
                    ...
                    <div class="tab-pane fade" id="experience" role="tabpanel" aria-labelledby="experience-tab">
                        ...
                        @isset($advocate)
                            @foreach ($advocate->experience as $exp)
                    ...
                    <div class="tab-pane fade" id="emergency-contact" role="tabpanel"
                        ...
                        @isset($advocate)
                            @foreach ($advocate->emergencyContact as $emc)
                    ...
                    <div class="tab-pane fade" id="address-form" role="tabpanel" aria-labelledby="address-form-tab">
                        @isset($advocate)
                            @foreach ($advocate->address as $address)
    ...
    <script>
        $(document).ready(function() {
            ...
            populateDropdown(
                ...
                'Select District',
                `{{ $advocate->address[0]->district_id ?? '' }}`,
                function() {
